...
namespace Application.Auth.Commands.VerifyOtp
{
        ...
        :IRequestHandler<VerifyOtpCommand, Result<VerifyOtpDto>>
    {
        ...
        public async Task<Result<VerifyOtpDto>> Handle(VerifyOtpCommand command,CancellationToken cancellationToken)
        {
            var result = await _auth.VerifyOtpAsync(command.phoneNumber, command.otp);
