...
namespace Domain.Aggregates.User
{
    ...
    public sealed class OTPCode : Entity<Guid>
    {
        ...
        private OTPCode(Guid id, Guid userId, PhoneNumber phoneNumber, string code, DateTimeOffset createdAt, DateTimeOffset expiresAt)
            : base(id)
        {
        ...
        public static OTPCode Issue(Guid userId, PhoneNumber phoneNumber, string code, DateTimeOffset now)
        {
            var expiresAt = now.AddMinutes(DefaultExpiryMinutes);
            return new OTPCode(Guid.NewGuid(), userId, phoneNumber, code, now, expiresAt);
        }

        public bool IsExpired(DateTimeOffset now) => now >= ExpiresAt;

        public bool CanBeUsed(DateTimeOffset now) => !IsUsed && !IsExpired(now);
